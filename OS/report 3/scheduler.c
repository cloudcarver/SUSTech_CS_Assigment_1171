...
void enqueue(struct waitqueue *newnode){
	if(head){
		struct waitqueue *p;
...
/* END OF linkedlist helper function */
...
#ifdef DEBUG		
		printf("\nbegin switch: current jid=%d and next jid=%d\n",
				 current->job->jid, next->job->jid);
#endif	
...
void do_enq(struct jobinfo *newjob, struct jobcmd enqcmd)
{
	...
	if (pid == 0) {
		...
		printf("Job %d begin running. ", newjob->jid);
...
void do_deq(struct jobcmd deqcmd)
{
	...
	if (current && current->job->jid == deqid) {
		...
		if (head) {
			...
			if(!select){
#ifdef DEBUG
				printf("No such job in scheduler.\n");
#endif
				return;
			}
			selectprev->next = select->next;
...
void do_stat()
{
	...
	*  2. job pid
	*  3. job name
	*  4. job current priority
	*  5. job default priority
	*  6. job owner
	*  7. job run time
	*  8. job wait time
	*  9. job create time
	...
	*/
	...
	printf( "JID\tPID\tNAME\tCURPRI\tDEFPRI\tOWNER\tRUNTIME\tWAITTIME\tCREATTIME\tSTATE\n");
